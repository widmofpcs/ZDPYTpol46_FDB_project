...
class InvoiceCustomerChoiceView(views.View):
    ...
    def get(self, request):
        ...
        if customer:
            task_qs_len = len(Task.objects.filter(customer=customer, is_active=False, invoiced=False))
            if not task_qs_len:
...
class InvoiceTaskChoiceView(views.View):
    ...
    def get(self, request):
        ...
        form = InvoiceCreateForm(initial={'customer': customer})
        form.fields['tasks'].queryset = Task.objects.filter(customer=customer, is_active=False, invoiced=False)
        form.fields['customer'].queryset = Customer.objects.filter(id=customer)
